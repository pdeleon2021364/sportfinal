 drop database if exists DBVeterinaria2025;
create database DBVeterinaria2025;
use DBVeterinaria2025;
...
Delimiter ;
call sp_actualizar_cita('1','2025-05-15','09:00:00','Vacunación anual','Completado','1','1');
...
CREATE PROCEDURE sp_recetas_con_mascotas_y_medicamentos()
BEGIN
    SELECT 
        v.nombreVeterinario AS Veterinario,
        m.nombreMascota AS Mascota,
        md.nombreMedicamento AS Medicamento,
        r.dosis AS Dosis,
        r.frecuencia AS Frecuencia,
        r.duracionDias AS DuracionDias
    FROM Receta r
    INNER JOIN Consulta c ON r.codigoConsulta = c.codigoConsulta
    INNER JOIN Mascota m ON c.codigoMascota = m.codigoMascota
    INNER JOIN Medicamento md ON r.codigoMedicamento = md.codigoMedicamento
    INNER JOIN Veterinario v ON c.codigoVeterinario = v.codigoVeterinario;
END $$
DELIMITER ;
CALL sp_recetas_con_mascotas_y_medicamentos();

DELIMITER $$

CREATE PROCEDURE sp_facturas_con_mascotas(IN p_codigoFactura INT)
BEGIN
    SELECT 
        ...
        m.nombreMascota AS Mascota,
        f.codigoCliente AS CodigoCliente,
        f.codigoEmpleado AS CodigoEmpleado
    FROM Factura f
    INNER JOIN Empleado e ON f.codigoEmpleado = e.codigoEmpleado
    INNER JOIN Cliente c ON f.codigoCliente = c.codigoCliente
    INNER JOIN Mascota m ON c.codigoCliente = m.codigoCliente
    WHERE f.codigoFactura = p_codigoFactura;
END$$

DELIMITER ;
-- drop procedure sp_facturas_con_mascotas;

DELIMITER $$
CREATE PROCEDURE sp_carnet_vacunacion()
BEGIN
    SELECT 
        v.nombreVacuna AS Vacuna,
        vac.fechaAplicacion AS FechaVacunacion,
        vet.nombreVeterinario AS Veterinario,
        m.nombreMascota AS Mascota,
        c.nombreCliente AS Dueño
    FROM Vacunacion vac
    INNER JOIN Vacuna v ON vac.codigoVacuna = v.codigoVacuna
    INNER JOIN Veterinario vet ON vac.codigoVeterinario = vet.codigoVeterinario
    INNER JOIN Mascota m ON vac.codigoMascota = m.codigoMascota
    INNER JOIN Cliente c ON m.codigoCliente = c.codigoCliente;
END$$
DELIMITER ;
CALL sp_carnet_vacunacion();

-- 15 REPORTES (PRIMEROS 20pts) 
-- 1.	Obtener el nombre del cliente, el nombre de su mascota y el motivo de la última consulta realizada.
Delimiter $$
	Create Procedure sp_reporte_uno()
    Begin
    Select C.nombreCliente as Cliente, M.nombreMascota as Mascota, CO.motivo as Motivo From Cliente C
    Inner Join Mascota M on C.codigoCliente = M.codigoCliente
    Inner Join Consulta CO on C.codigoCliente = CO.codigoConsulta
ORDER BY
    C.nombreCliente, M.nombreMascota;
    End$$
Delimiter ;
call sp_reporte_uno();

-- 2.	Mostrar el nombre del veterinario, el nombre del cliente y el nombre de la mascota atendida en cada cita completada.
Delimiter $$
	Create procedure sp_reporte_dos()
    Begin 
    Select V.nombreVeterinario, C.nombreCliente, M.nombreMascota, CI.estado
    From Cliente C
    Inner join Cita CI ON C.codigoCliente = CI.codigoCita 
   	Inner Join Mascota M ON C.codigoCliente = M.codigoCliente
    Inner Join Veterinario V ON M.codigoMascota = V.codigoVeterinario
	where CI.estado Like '%Completado%';
    End$$
Delimiter ;
call sp_reporte_dos();
Select * From Cita;

-- 3.	Listar las recetas indicando el medicamento, la consulta a la que pertenece y el veterinario que la prescribió
Delimiter $$
	Create procedure sp_reporte_tres()
    Begin
    Select R.codigoReceta,M.nombreMedicamento, CO.codigoConsulta, V.nombreVeterinario From Receta R
    Inner Join Medicamento M on R.codigoMedicamento = M.codigoMedicamento
    Inner Join Consulta CO on R.codigoConsulta = CO.codigoConsulta
    Inner Join Veterinario V ON CO.codigoVeterinario = V.codigoVeterinario;
    End$$
Delimiter ; 
call sp_reporte_tres();

-- 4. Mostrar las facturas emitidas, con nombre del cliente, nombre del empleado que las procesó y total facturado.
Delimiter $$
	Create procedure sp_reporte_cuatro()
    Begin
    Select F.codigoFactura, C.nombreCliente, E.nombreEmpleado, F.total From Factura F
    Inner Join Cliente C on F.codigoCliente = C.codigoCliente
    Inner Join Empleado E on F.codigoEmpleado = E.codigoEmpleado;
    End$$
Delimiter ;
call sp_reporte_cuatro();

-- 5. Consultar los tratamientos realizados con su diagnóstico relacionado y el veterinario que lo atendió.
Delimiter $$
	Create procedure sp_reporte_cinco()
    Begin	
    Select T.descripcion, CO.diagnostico, V.nombreVeterinario From Tratamiento T
    Inner Join Consulta CO on T.codigoConsulta = CO.codigoConsulta
    Inner Join Veterinario V on CO.codigoVeterinario = V.codigoVeterinario;
    End$$
Delimiter ;
call sp_reporte_cinco();

-- 6. Listar las vacunaciones indicando el nombre de la mascota, el nombre del veterinario y el nombre de la vacuna aplicada.
Delimiter $$
	Create procedure sp_reporte_seis()
    Begin	
    Select 
    M.nombreMascota,V.nombreVeterinario,VAC.nombreVacuna FROM Vacunacion VA
	Inner Join Mascota M ON VA.codigoMascota = M.codigoMascota
	Inner Join Veterinario V ON VA.codigoVeterinario = V.codigoVeterinario
	Inner Join Vacuna VAC ON VA.codigoVacuna = VAC.codigoVacuna;
    End$$
Delimiter ;
call sp_reporte_seis();

-- 7. Reportar las compras de medicamentos, indicando nombre del proveedor, nombre del medicamento y fecha de compra.

Delimiter $$
	Create procedure sp_reporte_siete()
    Begin 
    Select P.nombreProveedor, M.nombreMedicamento ,C.fechaCompra AS fecha_compra FROM Compra C
	Inner join Proveedor P ON C.codigoProveedor = P.codigoProveedor
    Inner Join Medicamento M ON P.codigoProveedor = M.codigoMedicamento;
    End$$
Delimiter ; 
call sp_reporte_siete();

-- 8. Mostrar las  mascotas con al menos una receta generada.
Delimiter $$
	Create Procedure sp_reporte_ocho()
    Begin
    Select M.nombreMascota, R.codigoReceta From Mascota M 
    Inner Join Receta R on R.codigoReceta = M.codigoMascota;
    End$$
Delimiter ;
call sp_reporte_ocho();

-- 9. Obtener un reporte de las citas pendientes con datos del cliente, la mascota y el veterinario asignado.
Delimiter $$
	Create procedure sp_reporte_nueve()
    Begin 
    Select V.nombreVeterinario, C.nombreCliente, M.nombreMascota, CI.estado
    From Cliente C
    Inner join Cita CI ON C.codigoCliente = CI.codigoCita 
   	Inner Join Mascota M ON C.codigoCliente = M.codigoCliente
    Inner Join Veterinario V ON M.codigoMascota = V.codigoVeterinario
	where CI.estado Like '%Pendiente%';
    End$$
Delimiter ;
call sp_reporte_nueve();
Select * From Cita;

-- 10. Mostrar el historial de consultas de cada mascota, incluyendo el veterinario y diagnóstico correspondiente.
Delimiter $$
	Create Procedure sp_reporte_diez()
	Begin
	Select M.nombreMascota, C.nombreCliente, CO.fechaConsulta, V.nombreVeterinario, CO.diagnostico, CO.motivo From Consulta CO
	inner Join Mascota M ON CO.codigoMascota = M.codigoMascota
	inner Join Cliente C ON M.codigoCliente = C.codigoCliente
	inner Join Veterinario V ON CO.codigoVeterinario = V.codigoVeterinario;
	End$$
Delimiter ;
call sp_reporte_diez();

-- 11. Generar un reporte de vacunas aplicadas, indicando fecha, mascota y proveedor del medicamento.
Delimiter $$
	Create procedure sp_reporte_once()
    Begin
	SELECT VA.fechaAplicacion,M.nombreMascota,P.nombreProveedor FROM Vacunacion VA
	Inner Join Mascota M ON VA.codigoMascota = M.codigoMascota
	Inner Join Vacuna V ON VA.codigoVacuna = V.codigoVacuna
	Inner Join Proveedor P ON V.codigoVacuna = P.codigoProveedor; 
	End$$
Delimiter ;
call sp_reporte_once();

-- 12. Mostrar todas las consultas realizadas, vinculando la mascota, el cliente y el veterinario.
Delimiter $$	
	Create Procedure sp_reporte_doce()
    Begin
    Select CO.codigoConsulta, CO.fechaConsulta, C.nombreCliente, C.apellidoCliente, M.nombreMascota, V.nombreVeterinario, CO.motivo, CO.diagnostico From Consulta CO
	Inner Join Mascota M ON CO.codigoMascota = M.codigoMascota
    Inner Join Cliente C ON M.codigoCliente = C.codigoCliente
    Inner Join Veterinario V ON CO.codigoVeterinario = V.codigoVeterinario;
    End $$
Delimiter ;
call sp_reporte_doce();

-- 13. Obtener las recetas médicas con el detalle de medicamento, tratamiento y veterinario correspondiente.
Delimiter $$
	Create procedure sp_reporte_trece()
    Begin
    SELECT
    R.codigoReceta, MED.nombreMedicamento, T.descripcion AS descripcion_tratamiento, V.nombreVeterinario FROM Receta R
	Inner Join Medicamento MED ON R.codigoMedicamento = MED.codigoMedicamento
    Inner Join Consulta CO ON R.codigoConsulta = CO.codigoConsulta
	Left Join Tratamiento T ON CO.codigoConsulta = T.codigoConsulta 
	Inner Join Veterinario V ON CO.codigoVeterinario = V.codigoVeterinario;
    End$$
Delimiter ;
call sp_reporte_trece();

-- 14. Listar todas las facturas, el cliente asociado, el empleado que la emitió y la mascota consultada relacionada.
Delimiter $$
	Create procedure sp_reporte_catorce()
    Begin
    SELECT F.codigoFactura, CL.nombreCliente, EM.nombreEmpleado, M.nombreMascota FROM Factura F
    Inner Join Cliente CL ON F.codigoCliente = CL.codigoCliente
	Inner Join Empleado EM ON F.codigoEmpleado = EM.codigoEmpleado
	Left Join Mascota M ON CL.codigoCliente = M.codigoCliente;
    End$$
Delimiter ;
call sp_reporte_catorce();

-- 15. Reportar todas las consultas, mostrando cliente, mascota y los medicamentos recetados en cada una.
Delimiter $$
	Create procedure sp_reporte_quince()
    Begin
    Select CO.codigoConsulta, CL.nombreCliente, MA.nombreMascota, VE.nombreVeterinario, CO.motivo, CO.diagnostico From Consulta CO
    Inner Join Mascota MA ON CO.codigoMascota = MA.codigoMascota
    Inner Join Cliente CL ON MA.codigoCliente = CL.codigoCliente
    Inner Join Veterinario VE ON CO.codigoVeterinario = VE.codigoVeterinario
    Left Join Receta RE ON CO.codigoConsulta = RE.codigoConsulta
    Left Join Medicamento ME ON RE.codigoMedicamento = ME.codigoMedicamento
GROUP BY
    CO.codigoConsulta, CO.fechaConsulta, CL.nombreCliente, MA.nombreMascota, VE.nombreVeterinario, CO.motivo, CO.diagnostico;
    End$$
Delimiter ;
call sp_reporte_quince();
